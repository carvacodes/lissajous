...
let lissaTimer = 0;
let lissas = [];
let updateSpeed = (Math.PI / 250) / sizeRatio;    // the relative distance through the curve each point should advance per frame
...
function animate(callbackTime) {
  ...
  // lissaTimer = lissaTimer >= 2 * Math.PI ? 0 : lissaTimer + (Math.PI / 360);
